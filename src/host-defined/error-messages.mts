...
'"arguments" cannot be used as an identifier in class static block'
  ...
  | 'A constructor declares no return type; the class is the type a construction yields'
  | 'A constructor yields its class and may return only `this`; write the expression as a statement followed by `return;`, or use a static method to return something else'
  ...
  | 'a positional argument after a named argument must continue a named rest'
  | 'a property with a declared type cannot be redefined as an accessor'
  ...
  | 'an array type takes a single type argument'
  ...
  | 'an integer width must be an integer from 1 through 65536'
  ...
  | 'no declared operator signature accepts these operands'
  ...
  | 'the named arguments do not satisfy the signature in view'
  | 'the operator is ambiguous between two declared signatures'
  | 'the placement extent exceeds the buffer'
  | 'the range is empty'
  | 'the reciprocal of zero is undefined'
  | 'the shift distance is negative'
  ...
  | 'this operator is not defined for a complex'
  ...
  | 'this reference names an array element that has been removed'
...
'$1'
  ...
  | '$1 is a generic class; it is constructed through a specialization, not through its declaration'
  ...
  | '$1 is a typed array and cannot be proxied'
  | '$1 is declared by this proxy\'s type and a trap may not deny it'
  | '$1 is declared by this proxy\'s type and a trap may not delete it'
  | '$1 is declared by this proxy\'s type and a trap may not omit it'
  ...
  | '$1 is not a range of this stream'
  | '$1 is not a rational'
  ...
  | '$1 is outside the compile-time-evaluable fragment, so a type cannot be computed from it'
  | '$1 is protected'
  | '$1 is too large'
  | '$1 is too small'
  ...
  | 'a $1 cannot be cloned'
  | 'a $1 clause is not satisfied by this application'
  | 'a call in a location-consuming context must return a ref, and $1 does not'
  | 'a closed type annotation could not be evaluated to a type: $1'
  ...
  | 'a literal zero divisor is not a division at $1'
  ...
  | 'a preprocessor module cannot import itself, directly or otherwise: $1'
  | 'a preprocessor module must evaluate synchronously: $1'
  | 'a preprocessor module must load synchronously: $1'
  | 'a shift distance of $1 is negative'
  ...
  | 'a tuple element default must be compile-time evaluable, and $1 is not'
  ...
  | 'the return type of $1 grows at every step and cannot be inferred; write it'
...
'"add" property ($1) of object $2 is not a function'
  ...
  | 'a shift distance of $1 shifts every bit out of $2'
  ...
  | 'the indexed access $1 has no declared property for $2'
  | 'the replacement decorator $1 is not compile-time evaluable: it names $2'
  | 'the type parameter $1 occurs in $2, incompatible with its declared variance'
  | 'the type parameter $1 of $2 has no argument and no default'
  | 'the type parameter $1 of $2 is not determined by the arguments and has no default; supply it explicitly with $2.<...>'
...
'"roundingMode" on object $1 is not valid ($2), only $3 are accepted'
  ...
  | '$1 is not assignable to $2, and it is the inferred return type of $3'
  | '$1 is not assignable to $2, and it is the inferred return type of $3, which is what $4 declares'
  | '$1 is not assignable to $2, and it is the inferred return type of $3, whose $4 comes from $5'
