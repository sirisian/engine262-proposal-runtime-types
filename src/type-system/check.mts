 ...
 *
 * `new Error("x")` is an `Error` whatever it
 ...
 *
 * At the types these already answer -
 ...
 * fixed `boolean` would displace an overload - a mistake made once
 ...
 *
 * Each is a predicate or a producer
...
const genericWhereChecks = new WeakMap<object, readonly GenericWhereCheck[]>();
export function GenericWhereChecksOf(root: object): readonly GenericWhereCheck[] {
  return genericWhereChecks.get(root) ?? [];
}
...
function conversionHasEffect(target: TypeRecord | null | undefined): boolean {
  ...
  // A boundary at a collection SPECIALIZATION does more than check.
 ...
 * conversion.
 *
 * This is the half `bigintTarget` was missing.
...
const ArrayMethodSignature = (name: string, element: TypeRecord, receiver: TypeRecord): Known => {
  ...
  switch (name) {
    ...
    //, so a foreign element raised no static error at all -
    ...
    case 'splice':
      ...
      // a malformed extent. Copying an error is not agreement.
      return {
        Kind: 'function',
        Signatures: [{
          Parameters: [
    ...
    // `concat` is NOT one of the five below.
    //
...
function CheckStatementList(statementList: readonly ParseNode[] | null, root: ParseNode, session?: CheckSession, afterTypeEvaluation = false): ObjectValue[] {
   ...
   * taken from the member the target declares.
   *
   ...
   *
   * An earlier registry was read by written name and never consulted scope, so
   * the checker answered with the intrinsic where the runtime, which walks the
   * scope chain, answered with the binding. That is a checker/runtime divergence
   * about what an annotation MEANS - the very defect the registry existed to
   * remove, reintroduced by it.
   *
   ...
   * than the function's OWN annotation.
   *
   ...
   *
   *   function f(): void { return "s"; }            must REFUSE - a body
   *                                                 contradicting its OWN
   *                                                 annotation
   *   type O = { m(): void };
   *   const o: O = { m() { return "s"; } };         must ACCEPT - the
   ...
   *
   * The elision condition, unchanged: not ~any~, not a literal, and
   ...
   *
   * It cannot live in `returnTypes`: a `return`
  ...
  const requireAssignable = (source: Known, target: Known) => {
    ...
    // applies: admitted exactly where the width represents it exactly,
    ...
    // literal's Base as `number`, a mislabel pinned elsewhere, so the Base name is
  ...
  const superMember = (node: ParseNode.SuperProperty) => {
    ...
    const base = (self as { Base?: TypeRecord } | null | undefined)?.Base;
  ...
  /**
   * Class DECLARATIONS by name, and their instance types built lazily and
   * memoized. Lazily, because a class's structure now includes what it
   ...
   *
   * The assumed half. A contract's facts live on the
  ...
  const interfaceTypeOf = (name: string): Known => {
    ...
    try {
    // EVERY declaration of the name contributes, base and `partial` alike,
    ...
    for (const { member, declarationIndex } of memberSources) {
      ...
      if (typeof key === 'string') {
        ...
        // This walk was added for the ACROSS case and excluded the
      ...
      if (tm.MethodSignature) {
        // A GENERIC method signature's annotations resolve under the
  ...
  const classTypeOf = (name: string): Known => {
    ...
    // Map.<string, uint8>()` consequently has no Static Type (the second
    // half). Returning `libraryTypeRecord(name)` gives `new Map()` the BARE
    // `Map`, and a bare nominal is not assignable to a specialization - so
   ...
   * carries the public fields as its [[Structure]], which is the same channel
   * an interface already uses. Private members are resolved separately by
   * privateMember through their lexical class declaration.
   */
  ...
  const classMemberWalk = (n: ParseNode, want: 'instance' | 'static', privateNames = false) => {
    ...
    for (const el of cls.ClassTail?.ClassBody ?? []) {
      ...
      if (el.type === 'MethodDefinition') {
        ...
        let usable = true;
        // A GENERIC method's annotations resolve under its own
  ...
  const privateMemberTypes = new Map<ParseNode, Partial<Record<'instance' | 'static', PrivateProperties>>>();

  // #sec-typed-classes: a private member's type belongs to its lexical private
  // name. It must not become a property of the public structural view.
  const privateMember = (node: ParseNode.MemberExpression): PrivateProperties[number] | undefined => {
    const key = node.PrivateIdentifier;
    if (!key) return undefined;
    for (let owner: ParseNode | undefined = key.parent ?? node; owner; owner = owner.parent) {
      if (owner.type !== 'ClassDeclaration' && owner.type !== 'ClassExpression') continue;
      const declaration = owner.ClassTail.ClassBody?.find((element) => 'ClassElementName' in element
        && element.ClassElementName?.type === 'PrivateIdentifier' && element.ClassElementName.name === key.name);
      if (!declaration) continue;
      const mode = (declaration as { static?: boolean }).static ? 'static' : 'instance';
      let cached = privateMemberTypes.get(owner);
      if (!cached) {
        cached = {};
        privateMemberTypes.set(owner, cached);
      }
      if (!cached[mode]) {
        cached[mode] = [];
      ...
      let receiver = staticType(node.MemberExpression);
      const seen = new Set<TypeRecord>();
      while (receiver?.Kind === 'nominal' && receiver.Declaration !== owner && !seen.has(receiver)) {
        seen.add(receiver);
        receiver = (receiver as TypeRecord & { Base?: TypeRecord }).Base ?? null;
      }
      if (receiver?.Kind === 'nominal' && receiver.Declaration === owner && receiver.Arguments.length > 0) {
        return {
          ...property,
          type: SubstituteTypeArguments(property.type, owner, receiver.Arguments),
          ...(property.writeType ? { writeType: SubstituteTypeArguments(property.writeType, owner, receiver.Arguments) } : {}),
        };
      }
      return property;
    }
  ...
  const classLayoutsChecked = new WeakSet<object>();
  const inlineFieldsOf = (type: TypeRecord): readonly InlineField[] | null | undefined => {
    ...
    if (declaration.ClassTail.ClassHeritage) {
      ...
      if (!base) return null;
      fields.push({ key: '[[Base]]', type: base });
    }
    for (const element of declaration.ClassTail.ClassBody ?? []) {
      if (element.type !== 'FieldDefinition' || element.static) continue;
      if (!element.TypeAnnotation || element.Decorators?.length) return null;
      const fieldType = resolveType(element.TypeAnnotation.Type);
      if (!fieldType || mentionsTypeParameter(fieldType)) return null;
      const key = element.ClassElementName;
      fields.push({ key: key.type === 'PropertyName' ? '[computed]' : (key as { name?: string, value?: string }).name ?? String((key as { value?: unknown }).value), type: fieldType });
    }
   ...
   *
   * Extracted so that the coverage is computed ONCE.
   ...
   * |TypeParameterDefault|, or null where any has none.
   *
    ...
    switch (node.type) {
      case 'TypeReference': {
        if (node.TypeName.MemberNames.length > 0 || node.TypeArguments) {
          ...
          // The checker resolved named arguments POSITIONALLY, so its judgment and
          ...
          if (!builtinTakesArguments
            ...
            // a base that declares type parameters is being APPLIED, so its
            // object-shaped argument is a type and not a record. `lookupAlias`
          ...
          // those arguments substituted. This path resolved a user CLASS,
          ...
          // A generic INTERFACE under written arguments. This path resolved
          // a user CLASS, an ALIAS and a builtin and NOT an interface, so
        ...
        // would shadow the completed one and the added member would silently
        // stop being checked.
        //
      ...
      // Closing the remaining resolver gaps. Each mirrors its arm of
      // `TypeNodeToTypeRecord` exactly - the record shape is the contract
      ...
      //
      // The last blocking entry in that gap list. What blocked it was not
      ...
      // unclaimed key failed. The runtime enforces it now, which needed a
      // composite shape to be built where the clause says it belongs first.
      // Three findings, one order.
      case 'PatternType':
      ...
      case 'IntersectionType': {
        ...
        // `E.<{ brand }> & uint8` is only reduced, and reaches the program as a
        // downstream error. A missed diagnostic, never a wrong one: the type is
        // `never` in both cases.
        //
      ...
      case 'ObjectType': {
        ...
        for (const member of node.TypeMemberList) {
          ...
          // read it differently: the RUN TIME requires a value to satisfy EVERY
          // member with the key, the checker only the FIRST, so `{ n: int32, n:
          ...
          // A METHOD member, `{ get(): uint8 }`. The parser gives a TypeMember
          // carrying a [[MethodSignature]] and a NULL TypeAnnotation, so it is
          // built here the way the INTERFACE path builds one rather than left
          // to the bail below, which would make the WHOLE object type resolve
          // to null - an annotation that is not checked at all, admitting `let
          // p: { get(): uint8 } = { get: "s" }`, a missing member, an excess
          // one, and any data member written beside the method. `{ get: () =>
          // uint8 }` spells the same record, and both display as `{ get: () =>
          // uint.<8> }`.
  ...
  const bindExplicitTypeArguments = (typeParams: readonly TypeParameterRecord[], argNodes: readonly ParseNode[], into: Map<string, TypeRecord>): void => {
    ...
    for (const a of argNodes) {
      if ((a as { IsSpread?: boolean }).IsSpread) {
        ...
        if (!t) {
          // A spread whose operand NAMES A VALUE binding
   ...
   *
   * `Promise.withResolvers` takes no
  ...
  const builtinStaticSignature = (callee: ParseNode | undefined): ((args: readonly ParseNode[], contextual?: Known) => Known) | undefined => {
    ...
    // A static whose result type is FIXED - it depends on nothing the
    ...
    if (base.name === 'Array' && method === 'fromAsync') {
      ...
      // machinery once budgeted for is not needed here.
      //
      ...
      return (args, contextual) => {
        ...
        // iterable argument's own elements adapt to.
    ...
    if (base.name === 'Array' && (method === 'from' || method === 'of')) {
      return (args, contextual) => {
        ...
        // `Array.from`'s argument is an ITERABLE rather than an element,
    ...
    if (base.name === 'Iterator' && method === 'from') {
      return (args, contextual) => {
        // The target's element, pushed through the ITERABLE argument.
    ...
    if (base.name === 'Promise') {
      ...
      const combinator: ((resolved: TypeRecord, rejected: TypeRecord | null) => Known) | undefined = {
        ...
        // `classTypeOf` does not know it - the same distinction the library
        // rule turned on. Resolved through `libraryTypeRecord`, which does.
      ...
      if (combinator) {
        return (args, contextual) => {
          ...
          // separating this arm from the other three iterable ones.
      ...
      if (method === 'reject') {
        ...
        // value, and `never` is the type of which there are none, so
        // `Promise.<never, uint8>` is the honest annotation and is accepted.
        return (args, contextual) => {
          // The target's REJECTION type, this argument being what the promise
          // rejects WITH - the mirror of `resolve` one arm below.
      ...
      if (method === 'resolve') {
        ...
        // contextual type and `any` without one.
        //
        // The rejection is `any` HERE because nothing in the call says what it
        // could be - and that is now harmless, where once it made the result
        // assignable to `Promise.<`R`, any>` and to no other instantiation.
        // #sec-generic-variance declares a promise `Promise.<out `R`, out `E`>`
        ...
        return (args, contextual) => {
          ...
          return value
          // The REJECTION type is ~never~, not ~any~, which is the exact
    ...
    // would drift.
    // `Reflect.typeOf`'s row is WITHDRAWN. #sec-reflect-typeof says
    ...
    // waits on a name for it rather than being written a different way. `isAssignable`
    ...
    if (base.name === 'Reflect' && method === 'typeOf') {
      // #table-typed-statics: `<T>(value: T): Reflect.TypeObject`.
      //
      // The row was WITHDRAWN once, written as `Reflect.Type`, because a
    ...
    if (base.name === 'Object' && (method === 'freeze' || method === 'seal' || method === 'preventExtensions')) {
      ...
      // lost. The fixed-result group listed these and they were never written - a fixed result
    ...
    if (base.name === 'Map' && method === 'groupBy') {
      return (args) => {
        ...
        // says the signature exists to state, and the two made
  ...
  const checkNumericCall = (call: ParseNode, contextual: Known): Known => {
    ...
    if (sawBigint) {
      // The bigint column resolves at run time.
      return null;
    }
    if (mixed) {
    ...
    for (const lit of literals) {
      ...
      // it cannot; the out-of-range-literal Early Error, uniformly.
  ...
  const typeArithmeticWithin = (node: ParseNode | null | undefined): void => {
    ...
    switch (node.type) {
      ...
      // `staticType`, which a bare statement never calls, so it is typed here
      // for the same reason the arithmetic above is: the check belongs to the
  ...
  const isNumericConstantExpression = (expr: ParseNode | null | undefined): boolean => {
    ...
    switch (e.type) {
      ...
      // A reference to a marked `const` is itself constant, so
   ...
   * and both come through here so that the arms are narrowed when their TYPE
   * is read - by a `return`, a declaration, an argument - and not only when the
   ...
   * be determined, in which case the binding stays untyped.
   *
  ...
  const staticTypeIn = (node: ParseNode | null | undefined, contextual: Known): Known => {
    ...
    // or `let o: { x: int32 } = ({ x: 1 });` would adapt against nothing and
    // be refused where the same value without the parentheses is accepted.
    // Parentheses do not change what an expression means and must not change
    // whether it is accepted.
    //
    // A CONCISE arrow returning an object literal depends on this: `() => ({
    // d: true })` must parenthesize to be an expression body at all, so every
    // such arrow arrives here wrapped, while the block form `() => { return {
    // d: true }; }` arrives with the literal itself.
    //
    ...
    if (node.type === 'TargetTypedNew') {
      if (!contextual || contextual.Kind === 'any') {
        ...
        // absent; the binding boundary resolves it when the declaration
        // evaluates, and the runtime arm reads it from the contextual stack the
        // declaration pushes. Reporting here would refuse a valid program
    ...
    // its parts against it. This is a standing pin, and until now the only
    // check on a literal's contents was the RUNTIME boundary: `let a:
    // [].<uint8> = [1, 300]` inside a never-called function raised nothing at
    // all, while `let x: uint8 = 300` had long been an Early Error.
    ...
    // exactly as it does at a binding.
    ...
    // An EMPTY array literal at a target no array can satisfy.
    //
    ...
    if (inner && inner.type === 'ObjectLiteral' && contextual) {
      ...
      // walk can run at all. Freshness was withheld there entirely -
      ...
      // are unioned - the OPPOSITE of the adaptation rule for the same target
      ...
      const intersectionArms = shape && shape.Kind === 'intersection'
        ...
        : [];
      // Arms of a UNION target whose freshness can be judged: every arm is
      ...
      const collectFreshArms = (t: unknown, seen: Set<unknown>, out: TypeRecord[]): void => {
        ...
        // An arm that is ITSELF a composite is flattened, not dropped (the
        // lesson): `({ x } | { y }) | { z }` denotes one three-arm union, and a
        // filter that kept only object arms reported `x` as excess. `seen`
        // guards a recursive alias, as the other walks do.
      ...
      // Merged BY KEY, not concatenated. Flat-mapping the arms put a key
      ...
      // `int32` alone and accepted, where `wantedOf` had refused it for
      // want of agreement between the arms. The merge introduced a second,
      // laxer path to a question already answered.
      //
      ...
      // arms UNION their keys - while no value satisfies it, which is what
      ...
      // below.
      //
      // #sec-literal-freshness is written for "an expected OBJECT TYPE"; a
      // union of them is expected object types too, so an excess property is
      // refused at a union as at any of its arms - at the top level, in a
      // nested member, across three arms, in an interface arm - rather than
      // surviving into the run time.
      //
      // This is REPORTED here and the arm below is NOT entered. Widening
      // `structural` to admit a union instead would accept `{ x: int32 } | {
      // x: string } = { x: 1 }`: that arm ends `return contextual`, so entering
      // it GIVES the literal the target's type and skips the `requireAssignable`
      // that refuses the disagreeing-arm row. Freshness has to be additive here,
      // not a new gate.
      //
      ...
      if (walkable && walkable.Kind === 'object') {
        ...
        // `mergeableArms` is the INTERSECTION case: the shape above is the
        // union of its arms, so it is as structural as an inline object type and
        // the same two rules apply to it; were the gate to admit a composite
        // with BOTH flags false, every check inside it would be disabled and
        // the MISSING-member rule silenced.
        //
        // An INTERFACE target is structural too. Its structure carries every
        // declaration of the name, `partial` ones included (`interfaceTypeOf`),
        // so a member a partial declares is not read as excess. Freshness
        // cannot be left to the boundary for an interface as it can for an
        ...
        // An INTERFACE target requires its members too. It is a
        ...
        // a class type at all is open, and widening here would answer it by
        // accident.
        const requiresMembers = (structural || targetIsInterface || mergeableArms)
          && (contextual as { Refinements?: readonly unknown[] }).Refinements === undefined;
        // A CLASS type is NOT satisfied by an object literal.
        //
        ...
        // as a shape, and it is the identity that its type is for." It is settled
        // the same rule from the `implements` side.
        //
        // The arm below returns `contextual` - the literal is GIVEN the target's
        // type and never compared against it - so a class target must be
        // refused BEFORE it: otherwise whatever the member walk does not catch
        // is accepted by construction, `let c: C = { }` with no member supplied
        // and `{ a: …, u: "s" }` with an excess one, and only a WRONG member
        // type is caught. The RUN TIME refuses it - `[object Object] is not
        // assignable to "C"` - and this gives the same verdict at the point the
        // arm skips.
        //
        ...
        // propagation: ungated, `{ a: number } & { a: 5 }` would be refused by
        // the `{ a: number }` arm while `{ a: 5 }` - the same interned type -
        // takes it.
    ...
    // large constants. #sec-literalvalueintype converts from "the
  ...
  const declaredOperator = (receiver: Known, key: string): Known => {
    ...
    while (receiver?.Kind === 'nominal' && !seen.has(receiver)) {
      ...
      if (definitions.length > 0 && declaration) {
        const pushed = pushTypeParameterScopeOf(declaration);
        try {
          const Signatures = definitions.map((operator) => {
            const Parameters = (operator.FormalParameters ?? []).map((formal) => {
            ...
            return {
              Parameters,
              Return: operator.TypeAnnotation ? resolveType(operator.TypeAnnotation.Type) : null,
              Untyped: !operator.TypeAnnotation && !(operator.FormalParameters ?? []).some((p) => (p as { TypeAnnotation?: unknown }).TypeAnnotation),
            };
          });
          const type = { Kind: 'function', Signatures } as Known;
          return SubstituteTypeArguments(type as TypeRecord, declaration, receiver.Arguments) as Known;
        } finally {
          if (pushed) typeParameterScopes.pop();
        }
      }
      receiver = (receiver as { Base?: Known }).Base ?? null;
    }
    return null;
  };
  ...
  const operatorResult = (callee: Known, args: readonly ParseNode[], node: ParseNode): Known => {
    ...
    if (callee?.Kind !== 'function') return null;
    let chosen = callee.Signatures.length === 1 ? callee.Signatures[0] : null;
    if (!chosen) {
      const types = args.map((arg) => staticType(arg));
      if (types.some((type) => !type || type.Kind === 'any' || mentionsTypeParameter(type))) return null;
      const candidates: OperatorSignature[] = callee.Signatures.map((signature) => ({
        Parameters: signature.Parameters, ReturnType: signature.Return ?? undefined,
        Function: Value.undefined, Untyped: (signature as { Untyped?: boolean }).Untyped,
      }));
      const resolution = resolveOverloadByTypes(candidates, types.map((type) => type!.Kind === 'literal' ? type!.Base : type!) as TypeRecord[],
        contextualCallTypes.get(node) ?? undefined);
      if (resolution.Kind !== 'resolved') {
        operatorFailures.set(node, resolution.Kind);
        return null;
      }
  ...
  const staticType = (node: ParseNode): Known => {
    ...
    switch (node.type) {
      ...
      case 'NumericLiteral': {
        ...
        // be used with an annotation AT ALL.
      ...
      case 'ArrayLiteral': {
        ...
        if (elements.length === 0) {
          // An EMPTY array literal is left UNTYPED.
      ...
      case 'CallExpression': {
        ...
        // library may carry a typed signature.
        //
        ...
        if (callee && callee.Kind === 'function' && callee.Signatures.length === 1) {
          ...
          if (only.TypeParameters && only.TypeParameters.length > 0) {
            ...
            const argNodes = spec?.type === 'TypeArgumentsExpression' ? spec.TypeArguments?.TypeArgumentList : undefined;
            {
              ...
              // #sec-variadic-parameters: a spread argument binds a PACK
      ...
      case 'MemberExpression': {
        ...
        if (m.Expression && m.MemberExpression) {
          const index = indexAccess(node);
          if (index?.get) {
            const result = operatorResult(index.get, index.arguments, node);
            return result?.Kind === 'reference' ? result.Target : result;
          }
          const key = memberKey(m);
        ...
        if (m.IdentifierName && m.MemberExpression) {
          ...
          // rejection type, and a handler's parameter is one of them.
          ...
          if (receiver && receiver.Kind === 'nominal' && receiver.Arguments.length > 0
            ...
            const protocol = iterationInterfaceRecord(receiver.LibraryName.startsWith('Async') ? 'AsyncIterator' : 'Iterator', receiver.Arguments);
            if (protocol?.Kind === 'object') {
              const method = protocol.Properties.find((property) => property.key === m.IdentifierName!.name);
      ...
      case 'NewExpression': {
        ...
        if (target && target.type === 'IdentifierReference') {
          ...
          // type to give it that the call supports - the same question settled
          // by resolving library nominals ONLY where type
        ...
        if (target && target.type === 'TypeArgumentsExpression') {
          ...
          if (spec.Expression.type === 'IdentifierReference') {
            ...
            // bare nominal is not assignable to a specialization - and that
      ...
      case 'BitwiseORExpression': {
        ...
        // #sec-user-defined-operators: dispatch precedes built-in arithmetic.
        const token = ({
          AdditiveExpression: (node as ParseNode.AdditiveExpression).operator,
          MultiplicativeExpression: (node as ParseNode.MultiplicativeExpression).MultiplicativeOperator,
   ...
   *
   * - TOO FEW. A trailing position may carry a default and a literal that
   ...
   */
  const expandValueSpreads = (elements: readonly ParseNode[]): readonly ParseNode[] => elements.flatMap((element) => {
    if (element.type !== 'SpreadElement' && element.type !== 'AssignmentRestElement') return [element];
    const contribution = StaticIterationContribution(staticType(element.AssignmentExpression), structureOf);
    return contribution.positions?.map((type) => typedExpressionView(element.AssignmentExpression, type)) ?? [element];
  });
  ...
  const checkArrayLiteralArityAgainstTuple = (node: ParseNode.ArrayLiteral, target: TypeRecord & { Kind: 'tuple' }) => {
    ...
    if (elements.some((element) => element.type === 'SpreadElement')) {
      const rest = target.Elements.find((position) => position.Rest);
      let minimumPosition = 0;
      let uncertain = false;
      for (const element of elements) {
        const spread = element.type === 'SpreadElement';
        const candidates = target.Elements.slice(minimumPosition).map((position) => position.Rest ? restElementType(position.Type) : position.Type);
        if (rest && minimumPosition >= target.Elements.length) candidates.push(restElementType(rest.Type));
      ...
      if (!rest && minimumPosition > target.Elements.length) report({ Kind: 'array', Element: anyTypeRecord, Extent: minimumPosition }, target);
      return;
    }
    ...
    //
    // Blocked twice on literal adaptation: the corpus spells a promise tuple
  ...
  const checkArrayLiteralAgainst = (node: ParseNode.ArrayLiteral, target: TypeRecord & { Kind: 'array' }) => {
    ...
    if (typeof target.Extent === 'number' && (spread ? count > target.Extent : count !== target.Extent)) {
  ...
  const checkObjectLiteralAgainst = (
    ...
    for (const member of node.PropertyDefinitionList ?? []) {
      ...
      if (!def.PropertyName && def.AssignmentExpression) {
        ...
        //, and only where this spread is the key's LAST writer.
      ...
      if (declared && def.AssignmentExpression) {
        ...
        //
        // Recorded rather than resolved into the signature, since [[ThisType]]
        // is contravariant and a real owner in the signature would refuse a
        // richer class where a
  ...
  /**
   * The literal's members for a MEMBERSHIP question.
   *
  ...
  const objectLiteralShape = (node: ParseNode | null | undefined): Known => {
    ...
    const wantedArmsFor = (key: string): readonly TypeRecord[] => {
      ...
      // A NESTED union contributes ITS arms. Flattening handles the record the RUN
    ...
    const wantedOf = (key: string): TypeRecord | null => {
      ...
      // every METHOD there without the return type its position wants.
      ...
      // Every arm is consulted, at any DEPTH.
      //
      ...
      // still be seen to disagree - the union rule left a disagreeing key REFUSED as an
      ...
      // discipline every arm of the substitution walk uses.
    ...
    for (const member of members) {
      ...
      // is compared. That is the asymmetry the nested-member rule turned on, one level in.
      ...
      // were invisible one level down and nothing adapted.
      //
      ...
      const armForMember = wantedForMember ?? ((): Known | null => {
        ...
        for (const arm of wantedArmsFor(key)) {
          ...
          const won = speculate(() => {
            ...
            // typed array is built - so requiring a non-null result made
            ...
            // which is the same test the object case arrived at.
      ...
      // the elements have been checked against it.
      //
      ...
      // where there is no target to adapt to. Widening unconditionally
      // made `let g: Grid = { t: 1.0, c: 1 }` build `{ t: number, c: number }`
      // and be refused, the untyped literals losing their adaptability before
      // the comparison - the literal-adaptation defect one level in, and this is its fix:
  ...
  const narrowingFactOf = (expr: ParseNode): { name: string, type: TypeRecord, negated: boolean, sense?: 'true' | 'false' } | undefined => {
    ...
    if (e.type === 'EqualityExpression') {
      ...
      // what makes a tagged union usable.
      for (const [subject, against] of sides) {
   ...
   * the paths that cannot produce one. Nothing in it is new - divergence is a
   * separate analysis, and the Early Errors of
   ...
   *
   * The callback rule could read a CONCISE arrow body, whose body IS the returned
   ...
   *    the same conservative test the return-boundary elision uses: a
  ...
  const inferredReturnType = (fn: ParseNode, parameterTypes: readonly Known[], wanted: Known = null, anchorage: { anchored: boolean, from?: string | null, origins?: { type: TypeRecord, from: string }[] } = { anchored: false }, mode: 'return' | 'yield' | 'resolve' = 'return'): Known => {
    ...
    // deep - `ConciseBody` holds an `ExpressionBody` which holds it.
  ...
  const checkIndexStore = (target: ParseNode, value: ParseNode): boolean => {
    const member = patternExpression(target);
    if (member?.type !== 'MemberExpression') return false;
    const access = indexAccess(member);
    if (access?.set) {
      operatorResult(access.set, [...access.arguments, value], target);
      return true;
    }
    if (access?.get) {
  ...
  const walkBindingElement = (b: ParseNode.SingleNameBinding | ParseNode.BindingElement) => {
    if (b.type === 'SingleNameBinding' && b.BindingIdentifier) {
      ...
      if (b.TypeAnnotation && declared && b.Initializer) {
        ...
        // target type, so the boundary has nothing to do.
  ...
  const enterFunction = (params: readonly ParseNode[] | null | undefined, returnAnnotation: ParseNode.TypeAnnotation | null | undefined, body: ParseNode | readonly ParseNode[] | null | undefined, checkReturns: boolean, contextual?: readonly Known[], generatorType?: Known, resumable?: boolean, contextualReturn?: Known | null) => {
    ...
    // contributes to R in Generator.<Y, R, N>. A bare yield annotation maps to
    ...
    const asyncResolution = resumable && !generatorType
      ...
      : null;
    // An `async` function's annotation types the PROMISE, so a
    ...
    if (body) {
      ...
      if (conciseReturn) {
        ...
        if (expr) {
          ...
          // ...and CHECKED against it. The contextual was recorded and
  ...
  if (callee && callee.Kind === 'function' && Array.isArray(c.Arguments)) {
    ...
    if (!sig && callee.Signatures.length > 1) {
      ...
      if (argTypes.every((t) => t !== null)) {
          // Preserve the parameter records used by argument binding.
        const candidates = callee.Signatures.map((s) => ({
          ...
          // The argument check binds these type parameters.
    ...
    if (sig) {
      ...
      if (mapped.spread && !mapped.named && chosen.Parameters.at(-1)?.Rest && chosen.Parameters.every((parameter) => !parameter.Optional)) {
        ...
        for (const argument of supplied) {
          const spread = argument.type === 'AssignmentRestElement';
          if (spread || uncertain) {
            const candidates = chosen.Parameters.slice(minimumPosition).map((parameter) => parameter.Rest ? restElementType(parameter.Type) : parameter.Type);
            const rest = chosen.Parameters.find((parameter) => parameter.Rest);
            if (rest && minimumPosition >= chosen.Parameters.length) candidates.push(restElementType(rest.Type));
            const wanted = candidates.reduce<Known>((joined, type) => joined ? joinTypes(joined, type) : type, null);
      ...
      // the runtime's SequenceAssignment rules for non-final and multiple
      // rests. Its entries carry the parameter slot and the offset within it.
      ...
      // as everywhere else.
      if (counts && !supplied.some((a) => (a as ParseNode).type === 'AssignmentRestElement')) {
      ...
      mapped.entries.forEach(({ node: arg, slot: slotIndex, offset }) => {
        ...
        if (param && param.Kind === 'function' && param.Signatures.length === 1
          ...
          // Continue to requireAssignable after recording contextual types:
  ...
  const walk = (node: ParseNode | readonly ParseNode[] | null | undefined): void => {
    ...
    switch (n.type) {
      ...
      case 'IsExpression': {
        ...
        // annotation does. The type-meta pin was thought to have flipped; it had
      ...
      case 'AssignmentExpression': {
        ...
        const declared = a.AssignmentOperator === '=' ? null
          : declaredOperator(staticType(a.LeftHandSideExpression), a.AssignmentOperator);
        if (declared) {
          operatorResult(declared, [a.AssignmentExpression], n);
        ...
        if (indexed?.type === 'MemberExpression') {
          const access = indexAccess(indexed);
          if (access?.set && a.AssignmentOperator === '=') {
            operatorResult(access.set, [...access.arguments, a.AssignmentExpression], n);
            walk(indexed.MemberExpression);
            walk(indexed.Expression);
            walk(a.AssignmentExpression);
            return;
          }
          if (access?.get && access.set && compoundChecksLikeAssignment(a.AssignmentOperator, staticType(indexed))) {
            const result = staticType(indexed);
            requireAssignable(staticTypeIn(a.AssignmentExpression, result), result);
            checkIndexStore(indexed, typedExpressionView(n, result));
          }
        ...
        if (judgedAssignmentOperator(a.AssignmentOperator) && a.LeftHandSideExpression.type === 'IdentifierReference') {
          ...
          // for the ~any~ path. A class instance has no structural type
      ...
      case 'ReturnStatement': {
        ...
        if (expr) {
          ...
          // produce none. An OWN annotation still refuses.
      ...
      case 'FunctionExpression': {
        ...
        //
        // What the parameter record then
