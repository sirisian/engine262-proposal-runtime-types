...
/**
 * Three of the four decisions PLAN-decorators-remaining.md §8 listed, settled
...
test('DECISION 3: an accessor\'s layout slot reports the DECLARED name', () => {
  // §2.1 settled that an accessor "participates in the memory layout exactly as
  ...
  expect(evaluated(`${cls} String(Reflect.getReflection.<Reflect.ClassField, A>("b").offset);`)).toBe('4');
  ...
  expect(evaluated(`${cls} String(Reflect.getReflection.<Reflect.ClassField, A>("c").offset);`)).toBe('8');
...
test('DECISION 4: a block decorator runs on every ENTRY', () => {
  // §6.2 recorded this as unanswered by the design; the engine had been
