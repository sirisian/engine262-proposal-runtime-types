...
/**
 * Callable Type Objects: the `T(v)` cast-call form and `Count(n)` enum
 * construction.
 *
 ...
 * (spec sec-conversions). A call on an enum type is the reverse conversion:
 * `Count(n)` returns the enumerator whose underlying value is `n`, and is a
 * TypeError when `n` is not one of them (spec sec-enums). Making a Type Object
 * callable does not change `typeof`, which reports "object" for a Type Object as
 * the proposal's feature detection (spec sec-reflect-typeof), nor its interned
