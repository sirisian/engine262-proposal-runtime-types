...
/**
 * Spec: #sec-check-insertion (Check Insertion) - the call and return
 * boundaries.
 *
 * An argument is checked against its parameter's declared type on the way in
 * and a returned value against the return annotation on the way out, so a
 * typed function's body may assume its annotations hold.
 */
...
test('annotated parameters convert at the call boundary', () => {
  ...
  // dead code #sec-narrowing forbids. Reading the boolean guards nothing
