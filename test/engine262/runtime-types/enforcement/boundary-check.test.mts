...
// Spec: #sec-the-boundary-check (The Boundary Check).
//
// The construction boundary. A bare value reaches a
...
test('a validate that refuses keeps the value out at the cast', () => {
  // The headline: the bound is enforced where the bare value tries
  // to come in.
...
test('the crossing scales by the conversion factor', () => {
  ...
  // agree), and the ratio's factor does the arithmetic.
