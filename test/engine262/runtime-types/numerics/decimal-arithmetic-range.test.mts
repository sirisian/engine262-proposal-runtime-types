 ...
 * width could hold it, so `decimal32.parse('9e90') * decimal32.parse('9e90')` answered
 * 8.1e181 and `decimal32.parse('9e96') + decimal32.parse('9e96')` answered 1.8e97 - values
...
test('a decimal result outside the width raises a RangeError', () => {
  expectThrownKind("let a = decimal32.parse('9e90'); let b = decimal32.parse('9e90'); a * b;", 'RangeError');
  expectThrownKind("let a = decimal32.parse('9e96'); let b = decimal32.parse('9e96'); a + b;", 'RangeError');
});
...
test('a wider decimal has the headroom its own range gives', () => {
  ...
  expect(evaluated("let a = decimal64.parse('9e90'); let b = decimal64.parse('9e90'); "
    + 'String(String(a * b).length);')).toBe('182');
});

test('arithmetic in range is unchanged, cohort and all', () => {
  expect(evaluated("let a = decimal32.parse('2.5'); let b = decimal32.parse('1.5'); String(a * b);")).toBe('3.75');
  // The quantum survives the addition, which is the reason these types exist.
  expect(evaluated("let a = decimal32.parse('2.5'); let b = decimal32.parse('1.5'); String(a + b);")).toBe('4.0');
  expect(evaluated("let a = decimal64.parse('5'); let b = decimal64.parse('3'); String(a - b);")).toBe('2');
  expect(evaluated("let a = decimal64.parse('2.5'); let b = decimal64.parse('1.5'); String(a / b);")).toBe('1.666666666666667');
  // And a zero divisor keeps its own error.
  expectThrownKind("let a = decimal64.parse('1'); let b = decimal64.parse('0'); a / b;", 'RangeError');
});

test('the two spellings of a string conversion agree', () => {
  // `parse` range-checked these digits and the conversion did not, so
  // `decimal32.parse('1e97')` built a value that `decimal32.parse('1e97')` refused.
  expectThrownKind("decimal32.parse('1e97');", 'RangeError');
  expectThrownKind("decimal32.parse('1e97');", 'RangeError');
  expect(evaluated("let d = decimal32.parse('1e96'); String(String(d).length);")).toBe('97');
});
