...
test('medium 4803 - Trim Right', () => {
  expectBuilderTrue(kit(`    function trimRight(s) { return literal(s.replace(/\\s+$/, '')); }
...
test('medium 2070 - Drop Char', () => {
  expectBuilderTrue(kit(`    function dropChar(s, c) { return literal(s.split(c).join('')); }
...
test('medium 3062 - Shift', () => {
  const f = ` function shift(T) { return tupleOf(elementTypes(T).slice(1)); }`;
...
test('medium 3192 - Reverse', () => {
  const f = ` function reverse(T) { return tupleOf(elementTypes(T).slice().reverse()); }`;
