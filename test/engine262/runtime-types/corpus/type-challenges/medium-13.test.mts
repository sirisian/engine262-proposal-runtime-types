...
test('medium 28333 - Public Type', () => {
  const f = `${TUP}
    function mapProperties(T, f2) { return objectOf(Reflect.getReflection(T).properties.map(f2).filter(p => p !== null)); }
...
test('medium 3376 - InorderTraversal', () => {
  const f = `${TUP}
    function field(node, name) { const p = Reflect.getReflection(node).properties.find(x => x.name === name); return p ? p.type : type null; }
    function walk(T) {
  ...
  const f = `${L}
    function combinationKey(items) {
      ...
      for (let i = 0; i < items.length; i += 1) {
        for (let j = i + 1; j < items.length; j += 1) { out.push(literal(items[i] + ' ' + items[j])); }
      }
...
test('medium 34857 - Defined Partial Record', () => {
  const f = `${TUP}
    function definedPartialRecord(K, V) {
