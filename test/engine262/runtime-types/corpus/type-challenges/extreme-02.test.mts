...
test('extreme 734 - Inclusive Range', () => {
  const f = ` function inclusiveRange(lo, hi) { const out = []; for (let i = lo; i <= hi; i += 1) { out.push(literal(i)); } return tupleOf(out); }`;
...
test('extreme 31447 - CountReversePairs', () => {
  const f = `    function countReversePairs(T) {
      const vals = Reflect.getReflection(T).elements.map(e => Reflect.getReflection(e.type).value);
      let count = 0;
      for (let i = 0; i < Number(vals.length); i += 1) { for (let j = i + 1; j < Number(vals.length); j += 1) { if (vals[i] > vals[j]) { count += 1; } } }
...
test('extreme 274 - Integers Comparator', () => {
  const f = ` function comparator(a, b) { return literal(a === b ? 0 : a < b ? -1 : 1); }`;
...
test('extreme 151 - Query String Parser', () => {
  const f = `    function parseQueryString(s) {
      ...
      return objectOf([...map].map(([key, values]) => ({ name: key, type: Number(values.length) === 1 ? literal(values[0]) : tupleOf(values.map(v => literal(v))), optional: false, readonly: false })));
    }`;
...
test('extreme 925 - Assert Array Index', () => {
  const f = ` function at(T, i) { return elementTypes(T)[i]; }`;
...
test('extreme 31997 - Parameter Intersection', () => {
  const f = `    function all(members) { return Reflect.makeType({ kind: 'intersection', members }); }
