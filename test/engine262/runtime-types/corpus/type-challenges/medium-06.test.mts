...
test('medium 2595 - PickByType', () => {
  expectBuilderTrue(kit(`    function pickByType(T, U) { return objectOf(Reflect.getReflection(T).properties.filter(p => p.type === U)); }
...
test('medium 2852 - OmitByType', () => {
  expectBuilderTrue(kit(`    function omitByType(T, U) { return objectOf(Reflect.getReflection(T).properties.filter(p => p.type !== U)); }
...
test('medium 5153 - IndexOf', () => {
  const f = ` function indexOf(T, U) { return literal(Reflect.getReflection(T).elements.findIndex(e => e.type === U)); }`;
...
test('medium 5140 - Trunc', () => {
  const f = `    function trunc(n) { const s = String(n); const i = s.indexOf('.'); return literal(i === -1 ? s : (s.slice(0, i) || '0')); }`;
