...
test('medium 9898 - Appear only once', () => {
  const f = `    function findEles(T) { const ts = elementTypes(T); return tupleOf(ts.filter(t => ts.indexOf(t) === ts.lastIndexOf(t))); }`;
...
test('medium 18220 - Filter', () => {
  const f = ` function filter(T, U) { return tupleOf(elementTypes(T).filter(t => Reflect.isAssignable(t, U))); }`;
...
test('medium 25170 - Replace First', () => {
  const f = `    function replaceFirst(T, S, R) {
...
test('medium 35191 - Trace', () => {
  const f = ` function trace(M) { const rows = elementTypes(M); return union(rows.map((r, i) => elementTypes(r)[i])); }`;
...
test('medium 2257 - MinusOne', () => {
  const f = ` function minusOne(n) { return literal(n - 1); }`;
...
test('medium 3243 - FlattenDepth', () => {
  const f = `    function flat(elements, d) {
