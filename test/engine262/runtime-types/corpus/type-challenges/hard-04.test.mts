...
test('hard 472 - Tuple to Enum Object', () => {
  const f = `    function enumObject(names, numeric = false) {
...
test('hard 2857 - IsRequiredKey', () => {
  const f = `    function isRequiredKey(T, K) {
...
test('hard 2949 - ObjectFromEntries', () => {
  const f = `    function objectFromEntries(T) {
...
test('hard 9160 - Assign', () => {
  const f = `    function assign(base, sources) {
      const map = new Map();
...
test('hard 13580 - Replace Union', () => {
  const f = `    function unionReplace(T, pairs) {
...
test('hard 14188 - Run-length encoding', () => {
  ...
  const dec = `    function decode(s) { return literal(s.replace(/(\\d*)(\\D)/g, (m, n, ch) => ch.repeat(n === '' ? 1 : Number(n)))); }`;
  expectBuilderTrue(kit(`${enc}\n String(encode('AAABCCXXXXXXY') === type '3AB2C6XY');`));
  expectBuilderTrue(kit(`${dec}\n String(decode('3AB2C6XY') === type 'AAABCCXXXXXXY');`));
});
...
test('hard 34286 - Take Elements', () => {
  const f = ` function take(n, T) { const els = elementTypes(T); return tupleOf(n >= 0 ? els.slice(0, n) : els.slice(n)); }`;
