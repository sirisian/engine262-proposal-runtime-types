...
test('hard 1383 - Camelize', () => {
  const f = `    function snakeToCamel(s) { return s.replace(/_(.)/g, (m, c) => c.toUpperCase()); }
    function camelize(T) {
...
test('hard 30178 - Unique Items', () => {
  const f = ` function uniqueItems(T) { const types = elementTypes(T); return tupleOf(types.map((t, i) => types.indexOf(t) === i ? t : never)); }`;
