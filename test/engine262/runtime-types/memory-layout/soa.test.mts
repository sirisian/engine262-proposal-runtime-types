...
test('array views alias bytes that already exist', () => {
  ...
  expect(evaluated(`${setup} const v = Span.<uint8>(buf); v[3] = 9; String(u8[3]);`)).toBe('9');
  // A wider element divides the extent.
  expect(evaluated(`${setup} String(Span.<uint32>(buf).length);`)).toBe('4');
  ...
  expect(evaluated(`${setup} const s = Span.<uint16>(buf, 1, undefined, 3); String(s.length) + "/" + String(Number(s[0]));`)).toBe('5/513');
  ...
  const rz = 'const rb = new ArrayBuffer(16, { maxByteLength: 64 }); const track = Span.<uint8>(rb); const fixed = Span.<uint8>(rb, 0, 8); ';
  ...
  expectThrownKind('const b = new ArrayBuffer(4); Span.<uint8>(b, 0, 8);', 'RangeError');
});
