...
// Spec: #sec-type-errors (Type Errors).
//
// The checking pass (src/type-system/check-pass.mts) runs per source text,
// after parse, before that source's evaluation. The synchronous
...
// only from this pass. A refused pair rejects the source before its first
// statement runs.
...
test('metadata subtype judgment: equal exponents admit a crossing between ratios', () => {
  ...
  expect(evaluated(`${dimensions}
    ...
    "ok";
  `)).toBe('ok');
  // The RUN crossing: the pass admits the pair, the construction admits the
...
test('an unclaimed-key pair is rejected at the parameterization, earlier and for the right reason', () => {
  // With no meta type declared the pairwise judgment would hold vacuously;
  // the unclaimed-key error is instead adjudicated in the checking pass
  // BEFORE the pairwise judgment, so this program is rejected at the
  // parameterization that writes `zork` and never reaches the judgment at
  // all. The vacuous case that
