...
test.each(['new (A)()', 'new A.<uint8>()', 'new (A.<uint8>)()'])(
  'abstractness survives wrapping and specialization: %s', (expression) => {
    expectStaticTypeError(`abstract class A<T: type = uint8> {} function f() { ${expression}; }`);
  },
);

test('specialized constructor arguments and type arity survive parentheses', () => {
  expectStaticTypeError('class Box<T: type> { constructor(x: T) {} } function f() { new (Box.<uint8>)("bad"); }');
  expectStaticTypeError('class Box<T: type> {} function f() { new (Box.<uint8, string>)(); }');
  expect(ok('class Box<T: type> { constructor(x: T) {} } new (Box.<uint8>)(1);')).toBe(true);
});

test('base contracts preserve generic substitution, overloads, and unknown arguments', () => {
  expectStaticTypeError('class B<T: type> { constructor(x: T) {} } class D extends B.<uint8> { constructor() { super("bad"); } }');
  expectStaticTypeError('class B<T: type> { constructor(x: T) {} } class D extends B.<uint8> {} new D("bad");');
