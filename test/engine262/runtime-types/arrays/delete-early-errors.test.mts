...
// #sec-typed-storage and #sec-type-errors. Only guaranteed fixed positions
...
test('growable arrays and tuple rest or default positions retain their timing', () => {
  expect(ok('function unused(a: [].<uint8>) { delete a[0]; }')).toBe(true);
  expect(ok('function unused(a: [uint8, ...[].<uint8>]) { delete a[2]; }')).toBe(true);
  expect(ok('function unused(a: [uint8 = 1]) { delete a[0]; }')).toBe(true);
});
