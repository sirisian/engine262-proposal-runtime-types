...
// Spec: #sec-contextual-types (Contextual Types), #sec-overload-resolution
// (Overload Resolution), #sec-literal-overload-ranking (Literal Overload
// Ranking): contextual types and overload resolution at a call, over the
// numeric library's listing. The checker resolves per
...
test('a contextual integer type resolves the call at that family', () => {
...
test('the checked integer row raises where the float path would wrap at the boundary', () => {
...
test('cast placement: converting the result and converting the operand differ', () => {
  // The numeric library listing's distinction, stated over the integer and
  // float rows: converting the RESULT keeps the integer row's exact square
  // root, converting the OPERAND selects the float row's approximation.
  ...
  // The same distinction over the bigint row, through the bigint-to-float
  // conversion source.
