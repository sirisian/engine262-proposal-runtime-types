...
// Spec: #sec-integer-operations (Integer Operations).
//
// Per-type arithmetic and wraparound: an operation on two values of an integer
// type produces a value of that type, wrapping at the type's width rather than
// decaying to plain `number`.
...
test('value-type identity is unaffected by the arithmetic rules', () => {
  // The literal adopts; the variable does not, and that is where identity
  // lives.
