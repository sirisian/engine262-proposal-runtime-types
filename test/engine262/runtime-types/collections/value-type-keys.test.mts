...
/**
 * VALUE TYPE CLASS KEYS.
 *
 * THESE ARE EXPECTED FAILURES, and they are written before the work they
 * describe, so that they convert to passes the moment value equality lands and
 * cannot be forgotten in the meantime. `test.fails` is vitest's inversion: the
 * test PASSES while its assertions fail, and starts FAILING the moment they
 * pass, which is the notification this file exists to produce.
 *
 * WHAT IS MISSING. `sec-equality-and-comparison` states that "two instances of a
 ...
 * since the fix will touch the same two operations.
 *
 ...
 * this is not "add an equality branch".
 *
 * IT IS OUT OF SCOPE FOR THE COLLECTIONS WORK. Nothing the typed collections do
 * moves any of these.
 */
...
test('an UNANNOTATED assignment copies', () => {
...
test('a value type copies when passed to a function', () => {
  expect(evaluated(`${V} function f(v: V) { v.x = (9 := uint32); } const a = new V(); f(a); String(a.x);`)).toBe('0');
});

test('reading an element out of a typed array copies it', () => {
...
// ---------------------------------------------------------------------------
// The collection consequences - what the typed collections actually need
// ---------------------------------------------------------------------------

test('a Map keyed on a value type class has one entry per VALUE', () => {
...
test('a Set of value type class instances dedups structurally', () => {
...
test('insertion COPIES the key, so mutating the original does not move it', () => {
...
test('comparison recurses by field KIND, not by byte image', () => {
  ...
  // fix that over-reaches is caught.
  expect(evaluated('class R { o: object | null = null; } String(new R() === new R());')).toBe('false');
});

// ---------------------------------------------------------------------------
// Controls - these pass today and must keep passing through any fix
// ---------------------------------------------------------------------------
...
test('control: a value type class is refused where identity is required', () => {
  ...
  // as a control so that a fix which introduces a real predicate does not
