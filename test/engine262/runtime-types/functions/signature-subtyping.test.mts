...
/**
 * Spec: #sec-function-type-identity-and-subtyping (Identity and Subtyping of
 * Function Types), by way of #sec-issignaturesubtype.
 *
 * Two conditions bite whether or not a rest is DECLARABLE, because a function
 * TYPE can always carry one:
 *
...
/**
 * A gap recorded rather than asserted.
 *
 ...
 * this file therefore goes through function TYPES, where the relation is
 * demonstrably right.
 *
 * The declaration route has to be reached before a rest can be DECLARED
 * rather than only written in a type. Recorded here rather than asserted,
 * since asserting the
...
test('a source with several rests relates by the assignment', () => {
  // This holds only once something can say which rest receives which
  // position; SequenceAssignment says, so
