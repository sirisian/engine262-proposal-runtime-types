...
test.each([
  ...
  'class C { m<T>(x: T): T { return x; } m<U>(x: U): U { return x; } }',
  'class C { m(x: uint8): void {} m(x: uint8, y: string = "s"): void {} }',
  'const c = { m(x: uint8): void {}, m(x: uint8): void {} };',
  'const c = { m<T>(x: T): T { return x; }, m<U>(x: U): U { return x; } };',
...
test('declared returns, constraints, and legacy overrides remain distinct', () => {
  ...
  expect(ok('class C { m<T extends string>(x: T): T { return x; } m<U extends uint8>(x: U): U { return x; } }')).toBe(true);
