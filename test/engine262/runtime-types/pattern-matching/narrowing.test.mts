...
test('a bare TYPE pattern narrows, as it always did', () => {
  ...
  expect(outcome('function f(v: uint8 | string) { if (v is uint8) { const s: string = v; return s; } return ""; } f(uint8(1));')).toBe('StaticTypeError');
});
...
test('`not` NEGATES the narrowing rather than abandoning it', () => {
  ...
  expect(outcome('function f(v: uint8 | string) { if (v is not uint8) { const n: uint8 = v; return n; } return uint8(0); } f("a");')).toBe('StaticTypeError');
...
test('a numeric literal takes the CONTEXTUAL TYPE of its position', () => {
  ...
  expect(outcome6('function f(v: uint8) { return match (v) { when 300: 1; default: 0; }; } f(uint8(1));')).toBe('StaticTypeError');
  ...
  // most exists for.
  expect(outcome6('function f(v: uint8) { return match (v) { when -1: 1; default: 0; }; } f(uint8(1));')).toBe('StaticTypeError');
