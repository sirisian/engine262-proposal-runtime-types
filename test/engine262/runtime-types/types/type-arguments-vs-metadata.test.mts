...
test('a generic alias applied to ONE object type is an application', () => {
  ...
  expect(kind('type Box<T: type> = { value: T }; type T = Box.<{ a: uint8 }>;')).toBe('object');
  expect(kind('type Id<T: type> = T; type T = Id.<{ a: uint8 }>;')).toBe('object');
  expect(kind('type Box<T: type> = { value: T }; type T = Box.<Box.<uint8>>;')).toBe('object');
  ...
  expect(kind('type Box<T: type> = { v: T }; type Inner = Box.<uint8>; type T = Box.<Inner>;')).toBe('object');
});
...
test('a brand over a type parameter SURVIVES instantiation', () => {
  ...
  expect(kind("type F<T: type> = T.<{ brand: 'B' }>; type T = F.<string>;")).toBe('parameterized');
  expect(evaluated("type F<T: type> = T.<{ brand: 'B' }>; type G = F.<string>;"
...
test('metadata still reads as metadata on every base that accepts it', () => {
  ...
  expect(kind("type Box<T: type> = { v: T }; type T = Box.<uint8>.<{ brand: 'V' }>;")).toBe('parameterized');
  expect(kind("class C<T: type> { v: T; constructor(v: T) { this.v = v; } } type T = C.<uint8>.<{ brand: 'V' }>;")).toBe('parameterized');
  expect(kind("type E = string.<{ brand: 'E' }>; type T = E.<{ brand: 'N' }>;")).toBe('parameterized');
});

test('type arguments still read as type arguments', () => {
  expect(kind('type Box<T: type> = { v: T }; type T = Box.<uint8>;')).toBe('object');
  expect(kind('type Box<T: type> = { v: T }; type T = Box.<[].<uint8>>;')).toBe('object');
  expect(kind('interface I { a: uint8 } type Box<T: type> = { v: T }; type T = Box.<I>;')).toBe('object');
  expect(kind('type Box<Element: type> = { v: Element }; type T = Box.<Element: uint8>;')).toBe('object');
...
test('ARITY stops mattering, and the two-object case is why', () => {
  ...
  expect(kind('type Pair<A: type, B: type> = { a: A, b: B }; type T = Pair.<{ x: uint8 }, { y: uint8 }>;')).toBe('object');
  expect(kind('type Pair<A: type, B: type> = { a: A, b: B }; type T = Pair.<uint8, string>;')).toBe('object');
});
...
test('expression position is unaffected', () => {
  ...
  expect(evaluated('function f<T: type>(x: T): T { return x; }'
...
test('an empty argument list applies every default', () => {
  ...
  expect(kind('type Grid<T: type = float64> = { v: T }; type T = Grid.<>;')).toBe('object');
  expect(kind("type Grid<T: type = float64> = { v: T }; type T = Grid.<>.<{ brand: 'V' }>;")).toBe('parameterized');
  expect(evaluated('type Grid<T: type = float64> = { v: T }; type A = Grid.<>;'
  ...
  expectThrown('type Box<T: type> = { v: T }; type T = Box.<>;');
});
...
test('the deferred reading produces the SAME type the written form does', () => {
  ...
  expect(evaluated("type F<T: type> = T.<{ brand: 'B' }>; type G = F.<uint8>;"
  ...
  expectThrown("type F<T: type> = T.<{ brand: 'B' }>; type G = F.<string>; let y: G = 'a';");
  expect(evaluated("type F<T: type> = T.<{ brand: 'B' }>; type G = F.<string>; String(G('a'));")).toBe('a');
});
...
test('a HIGHER-KINDED parameter is applied, not read as metadata', () => {
  ...
  expect(evaluated('type Identity<T: type> = T; class C<W<_>: type> { v: W.<uint8>; }'
    + " String('ok');")).toBe('ok');
});
...
test('the fully-defaulted generic keeps a route to a brand', () => {
  ...
  expect(kind("type Grid<T: type = float64> = { v: T }; type T = Grid.<float64>.<{ brand: 'V' }>;")).toBe('parameterized');
});
