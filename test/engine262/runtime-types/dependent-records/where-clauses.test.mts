...
import {
  evaluated, expectThrown, expectErrorFlagOff, expectStaticTypeError,
} from '../harness.mts';
...
test('a where clause is checked at each specialization', () => {
  ...
  expectStaticTypeError('function t<U: uint32>(x: uint32): uint32 where U < 8 where U > 2 { return x; } t.<1>(7);');
});
