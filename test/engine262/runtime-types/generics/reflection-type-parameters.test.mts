...
test('displayType prints the type parameter list and the pack marker', () => {
  expect(evaluated('type Id = <T: type>(x: T) => T; String(Id);')).toBe('<T: type>(x: T) => T');
  expect(evaluated('function count<...Ts: [].<type>>(...xs: Ts): uint32 { return 1; } String(Reflect.typeOf(count));')).toBe('<...Ts: [].<type>>(...xs: Ts) => uint.<32>');
