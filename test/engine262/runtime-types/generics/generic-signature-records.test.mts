...
test('a literal that adopts a generic contextual signature is a generic value', () => {
  ...
  expect(evaluated('interface J { map<T>(x: T): T; } let o: J = { map(x) { return x; } }; String(o.map.<uint8>((1 := uint8)));')).toBe('1');
  expect(evaluated("type I = { map<T>(x: T): T }; let o: I = { map(x) { return x; } }; String(o.map.<string>('s'));")).toBe('s');
  // A method that declares its own keeps its own.
  expect(evaluated('interface J { map<T>(x: T): T; } let o: J = { map<U>(x: U): U { return x; } }; String(o.map.<uint8>((2 := uint8)));')).toBe('2');
...
test('an interface method signature carries its type parameters at run time', () => {
  ...
  expect(evaluated('interface J { map<T>(x: T): T; } interface K { map<U>(x: U): U; } String((type J) === (type K));')).toBe('false');
  expect(evaluated('interface J { map<T>(x: T): T; } function take(j: J) { return j.map.<uint8>((3 := uint8)); } String(take({ map(x) { return x; } }));')).toBe('3');
});
