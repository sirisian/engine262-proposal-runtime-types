...
test('a bare type pack binds the tuple of a rest parameter\'s arguments (G1)', () => {
  expect(evaluated('function count<...Ts>(...xs: Ts): uint64 { return xs.length; } String(count(1, "a", true));')).toBe('3');
  expect(evaluated('function count<...Ts>(...xs: Ts): uint64 { return xs.length; } String(count());')).toBe('0');
});

test('a bounded type pack binds and its bound refuses (G2)', () => {
  expect(evaluated('function only<...Ts extends [].<string>>(...xs: Ts): uint64 { return xs.length; } String(only("a", "b"));')).toBe('2');
  expectThrown('function only<...Ts extends [].<string>>(...xs: Ts): uint64 { return xs.length; } only("a", 1);', 'not assignable');
});
...
test('a whole-tuple parameter binds a pack from one tuple (G4)', () => {
  expect(evaluated('function w<...Ts>(t: Ts): uint64 { return t.length; } String(w([1, "a"]));')).toBe('2');
});
...
test('explicit type arguments bind a pack, and the arguments are checked against its elements (G6)', () => {
  expect(evaluated('function count<...Ts>(...xs: Ts): uint64 { return xs.length; } String(count.<uint8, string>(1, "a"));')).toBe('2');
  expectThrown('function count<...Ts>(...xs: Ts): uint64 { return xs.length; } count.<uint8, string>("a", 1);', 'not assignable');
  expect(evaluated('type Two = [uint8, string]; function count<...Ts>(...xs: Ts): uint64 { return xs.length; } String(count.<...Two>(1, "a"));')).toBe('2');
});

test('a scalar parameter over an array rest keeps its element binding (regression guard)', () => {
  expect(evaluated('function all<T>(...xs: [].<T>): uint64 { return xs.length; } String(all(1, 2, 3));')).toBe('3');
});
