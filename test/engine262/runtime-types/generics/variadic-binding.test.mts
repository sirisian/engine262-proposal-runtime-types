// A variadic parameter BINDS at an application (spec.emu #sec-bindtypearguments,
...
test('an empty pack is the empty tuple', () => {
  expect(evaluated(`${V} String(new vec.<uint8, 4>().swizzle.<>());`)).toBe('0');
});

test('a tuple default fills an empty pack', () => {
  // The cause was inference running before the explicit frame was consulted
...
test('the element bound admits and refuses; a fixed extent is enforced', () => {
...
test('two packs split by their element bounds; a fixed parameter after a pack is reached', () => {
...
test('a named pack opens a run; a name reaches past a greedy pack', () => {
...
test('a spread splices a tuple before binding, to one specialization', () => {
