...
test('§1.3 the identities hold', () => {
  okSrc('function q<T: type>(o: T, k: keyof T): keyof T { return k; }');
  okSrc('function f<T: type>(x: T): T { return x; }');
});

test('§1.3 the call boundary is unchanged', () => {
  okSrc('function id<T: type>(v: T) { return v; } id(5); id("hi"); id({});');
});
...
test('the deferred access is TYPED, not merely unresolvable', () => {
  ...
  expectError('function p<T: type, K: keyof T>(o: T, k: K) { let v: uint8 = o[k]; }');
...
test('the two RESOLVERS agree about the deferred case', () => {
  ...
  okSrc('class Box<T: type, K: keyof T> { v: T[K]; }');
  okSrc('class Box<T: type> { v: keyof T; }');
  okSrc('class Box<T: type> { v: T; }');
});
...
test('the deferred case reaches inside a UNION operand', () => {
  ...
  expectError('function p<T: type>(o: T) { let v: (T | { n: uint8 })["n"] = 1; }');
});
