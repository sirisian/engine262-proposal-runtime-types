...
test('a type parameter is enforced at the CALL', () => {
  ...
  expectError('function f<T: type>(x: T): void {} f.<string>(Symbol());');
  expectError('function f<T: type extends string>(x: T): void {} f.<symbol>(Symbol());');
  okSrc('function f<T: type extends string>(x: T): void {} f.<string>("s");');
});
...
test('a generic body that is correct still runs', () => {
  expect(evaluated('function p<T: type, K: keyof T>(o: T, k: K): T[K] { return o[k]; } let u = { n: "x" }; String(p(u, "n"));')).toBe('x');
});
...
test('a value OF THE BOUND is not a value of the parameter', () => {
  ...
  expectError('function f<T: type>(x: T) { let v: T = 5; }');
});

test('but a T IS a T, which is what makes a generic body writable', () => {
  okSrc('function f<T: type>(x: T) { let v: T = x; }');
  okSrc('function f<T: type>(x: T): T { return x; }');
  okSrc('function p<T: type, K: keyof T>(o: T, k: K): T[K] { return o[k]; }');
});
...
test('a body annotation should be evaluated at each specialization', () => {
  ...
  // binds `T` to `string`.
  expectError('function f<T: type>(x: T) { let v: T = 5; } let s: string = "a"; f(s);');
});

test('a return should be evaluated at each specialization', () => {
  expectError('function f<T: type>(x: T): T { return 5; } let s: string = "a"; f(s);');
});
...
test('an indexed access over a parameter is deferred, not unresolvable', () => {
  ...
  expectError('function p<T: type, K: keyof T>(o: T, k: K): T[K] { return 5; }');
  expectError('function p<T: type, K: keyof T>(o: T, k: K) { let v: T[K] = 5; }');
  ...
  okSrc('function p<T: type, K: keyof T>(o: T, k: K): T[K] { return o[k]; }');
  okSrc('function p<T: type, K: keyof T>(o: T, k: K) { let v: T[K] = o[k]; }');
  expectError('function p<T: type, K: keyof T>(o: T, k: K) { let v: string = o[k]; }');
});
