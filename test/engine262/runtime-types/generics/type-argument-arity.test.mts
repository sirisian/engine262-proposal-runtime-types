...
test('a call supplying the wrong number of type arguments is refused', () => {
  expectThrown(dead('function f<T: type>(x: T) {} f.<uint8, string>(uint8(1));'),
    'type arguments');
  expectThrown(dead('function f<T: type>(x: T) {} f.<uint8, string, boolean>(uint8(1));'),
    'type arguments');
  // Too FEW is the same rule read from the other end.
  expectThrown(dead('function f<T: type, U: type>(x: T, y: U) {} f.<uint8>(uint8(1), uint8(2));'),
    'type arguments');
  // A method's type parameters are the function's.
  expectThrown(dead('class C { m<T: type>(x: T) { } } new C().m.<uint8, string>(uint8(1));'),
    'type arguments');
});

test('a construction is counted against the class, as a call is against the function', () => {
  expectThrown(dead('class Box<T: type> { v: T; constructor(v: T) { this.v = v; } }'
...
test('what the count does not reach', () => {
  ...
  expect(ok(dead('function f<T: type>(x: T) {} f.<uint8>(uint8(1));'))).toBe(true);
  expect(ok(dead('function f<T: type>(x: T) {} f(uint8(1));'))).toBe(true);
  expect(ok(dead('class Box<T: type> { v: T; constructor(v: T) { this.v = v; } }'
  ...
  expect(ok(dead('function f<T: type, U: type = string>(x: T) {} f.<uint8>(uint8(1));'))).toBe(true);
  expect(ok(dead('function f<T: type, U: type = string>(x: T) {} f.<uint8, boolean>(uint8(1));'))).toBe(true);

  // A PACK takes any number, so the declaration has no fixed arity to compare
  // against and the count is not judged.
  expect(ok(dead('function f<...T: [].<type>>(...x: T) {} f.<uint8, string, boolean>(uint8(1), "s", true);'))).toBe(true);
  // Once supplied, the pack is a tuple: the value arguments must fill its slots.
  expectStaticTypeError(dead('function f<...T: [].<type>>(...x: T) {} f.<uint8, string, boolean>(uint8(1));'));
  ...
  // report.
  expect(ok(dead('function f<T: type, U: type = string>(x: T) {} f.<U: boolean, T: uint8>(uint8(1));'))).toBe(true);
