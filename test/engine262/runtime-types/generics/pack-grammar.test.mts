// PLAN-variadic-and-named-generic-arguments.md Phase 3: the grammar of
// variadic parameters, spread arguments, generic function types, call
...
// #sec-object-types). Binding a pack at an application is Phase 4; this file
// applies nothing variadic.
...
test('E4 per run: a pack counts as defaulted and restarts the run', () => {
...
test('E1 syntactic half: adjacent packs where the first is unconstrained', () => {
...
test('a generic function type parses and resolves its own parameters (F-F)', () => {
...
test('an object type of call signatures is the function type they form (F-F)', () => {
...
test('`ref` distributes over a rest in function types and declarations (OQ-6)', () => {
  expect(evaluated('type Each = (e: uint32, ref ...rs: [].<uint32>) => void; "ok";')).toBe('ok');
  expect(evaluated('type Cb = (ref ...[].<uint32>) => void; "ok";')).toBe('ok');
  expect(evaluated('function f(ref ...xs: [].<uint32>) {} "declared";')).toBe('declared');   // binding semantics: Phase 4
});

test('a spread type argument parses (bound by Phase 4)', () => {
