...
test('per run: a pack counts as defaulted and restarts the run', () => {
  expect(evaluated('function f<T: type = uint8, ...I: [].<uint32>>(): uint32 { return 1; } "ok";')).toBe('ok');
  expect(evaluated('function g<...I: [].<uint32>, N: uint32>(): uint32 { return N; } "ok";')).toBe('ok');
  expectThrown('function h<...I: [].<uint32>, T: type = uint8, N: uint32>(): uint32 { return N; }');
  expectThrown('function k<T: type = uint8, N: uint32>(): uint32 { return N; }');   // the plain rule, unchanged
});

test('the syntactic half: adjacent packs where the first is unconstrained', () => {
  expectThrown('function f<...A: [].<type>, ...B: [].<type>>() {}', 'nothing typed');
...
test('a variance modifier precedes the pack marker', () => {
  expect(evaluated('interface Tup<out ...Ts: [].<type> extends [].<any>> {} "ok";')).toBe('ok');
});

test('a generic function type parses and resolves its own parameters', () => {
  expect(evaluated('type Id = <T: type>(x: T) => T; "ok";')).toBe('ok');
  expect(evaluated('class Event {} type Bounded = <T: type extends Event>(e: T) => void; "ok";')).toBe('ok');
  // (a `let` with a function-type annotation needs an initializer - a pre-existing rule - so the forms are aliases)
  expect(evaluated('type G = { g: <T: type>(x: T) => T }; "ok";')).toBe('ok');
});
...
test('an object type of call signatures is the function type they form', () => {
  ...
  expect(evaluated('type Gen = { <T: type>(x: T): T }; "ok";')).toBe('ok');
