// The integrated stress programs, each combining features the other files
// isolate. A failure here while those stay green is a COMPOSITION bug - the
// likeliest kind after two resolvers. What remains owed is pinned as
...
test('a value pack drives a method, checked by a where over the CLASS parameter', () => {
...
test('spread and specialization-as-value compose - one specialization, receiver-independent', () => {
...
test('the pack reflects with its variadic flag', () => {
...
test('a pack infers from ref-rest arguments, and a ref run forwards', () => {
...
test('specializations as values are interned, keyed, and typed', () => {
...
test('a class satisfies a generic interface by shape under its own parameter names, via implements', () => {
...
test('a specialization chain grows a tuple by splicing', () => {
...
test('polymorphic recursion is stopped by the budget, not the stack', () => {
...
test('identity is the ordered bindings, never the spelling', () => {
...
test('a spread of a dynamic array TYPE is refused statically', () => {
  // Without running the program: the body of `u` is never evaluated.
  expectThrown(`${STRESS} function u() { return stress.<uint8, ...[].<uint32>, N: 4>(); }`, 'stated extent');
});

test('a spread naming a value is refused statically', () => {
  expectThrown(`${STRESS} function u(xs: [].<uint32>) { return stress.<uint8, ...xs, N: 4>(); }`, 'the value xs');
});

test('a default reads an earlier pack - class form', () => {
...
test('a default reads an earlier pack - function form', () => {
