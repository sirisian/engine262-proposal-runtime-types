...
test('a field initializer reaches the specialization', () => {
  ...
  expect(evaluated('class C<T: type> { f = (1 := T); } String(new C.<uint8>().f is uint8);')).toBe('true');
});
...
test('a higher-kinded parameter keeps the nominal path', () => {
  ...
  expect(evaluated('type Identity<T: type> = T; class B<W<_>: type> {}'
...
test('a call may supply its type arguments explicitly', () => {
  ...
  expect(evaluated('function f<T: type>() { return (1 := T) is uint8; } String(f.<uint8>());')).toBe('true');
...
test('explicit type arguments take precedence over inference', () => {
  expect(evaluated('function f<T: type>(v: T) { return Reflect.typeOf(v); } String(typeof f.<uint8>((1 := uint8)));')).toBe('object');
  // inference alone is unchanged
  expect(evaluated('function id<T: type>(v: T): T { return v; } String(id(5));')).toBe('5');
});
...
test('a TYPE parameter reaches the same bodies as a value parameter', () => {
  expect(evaluated('class C<T: type> { m() { return (1 := T) is uint8; } } String(new C.<uint8>().m());')).toBe('true');
  expect(evaluated('class C<T: type> { static m() { return (1 := T) is uint8; } } String((C.<uint8>).m());')).toBe('true');
  expect(evaluated('class C<T: type> { constructor() { this.v = (1 := T); } }'
    + ' String(new C.<uint8>().v is uint8);')).toBe('true');
});

test('specialization identity distinguishes value and type arguments', () => {
  expect(evaluated('class C<T: type> { } String((C.<uint8>) === (C.<uint8>));')).toBe('true');
  expect(evaluated('class C<T: type> { } String((C.<uint8>) === (C.<uint16>));')).toBe('false');
  // a value argument and a type argument are different arguments
  expect(evaluated('class C<W: type> { } String((C.<4>) === (C.<uint8>));')).toBe('false');
});
...
test('a specialization satisfies an annotation naming the same application', () => {
  ...
  expect(evaluated('class B<T: type> {} const b: B.<uint8> = new B.<uint8>(); String(typeof b);')).toBe('object');
});
...
test('an accessor may not declare type parameters', () => {
  ...
  expectError('class C { get p<T: type>() { return 1; } } "ran";');
  expectError('class C { set p<T: type>(v) { } } "ran";');
  expectError('const o = { get p<T: type>() { return 1; } }; "ran";');
  expectError('const o = { set p<T: type>(v) { } }; "ran";');
  expectError('class C { static get p<T: type>() { return 1; } } "ran";');
});
...
test('an accessor keeps everything else it could already do', () => {
  ...
  expect(evaluated('class C<T: type> { get p() { return (1 := T) is uint8; } } String(new C.<uint8>().p);')).toBe('true');
...
test('a method\'s type arguments may be explicit or inferred', () => {
  expect(evaluated('class C { m<T: type>() { return (1 := T) is uint8; } } String(new C().m.<uint8>());')).toBe('true');
  // inferred from the call's arguments, as for a function
  expect(evaluated('class C { t<T: type>(v: T) { return (1 := T) is uint8; } } String(new C().t((1 := uint8)));')).toBe('true');
...
test('a method\'s parameters and its class\'s coexist', () => {
  ...
  expect(evaluated('class C<W: uint32> { m<T: type>() { return String(W) + ":" + String((1 := T) is uint8); } }'
    + ' String(new C.<4>().m.<uint8>());')).toBe('4:true');
  // and the method\'s shadows the class\'s where the names collide
  expect(evaluated('class C<T: type> { m<T: type>() { return (1 := T) is uint16; } }'
...
test('a method\'s own parameter may annotate its signature', () => {
  ...
  // runtime boundary.
  expectThrown('class C { m<T: type>(v: T) { return v; } } new C().m.<uint8>("x");', 'not assignable');
});
...
test('a type parameter default binds where no argument is supplied', () => {
  ...
  expect(evaluated('function f<T: type = uint8>() { return (1 := T) is uint8; } String(f());')).toBe('true');
  expect(evaluated('class C { m<T: type = uint8>() { return (1 := T) is uint8; } } String(new C().m());')).toBe('true');
  // where it already worked, it still does
  expect(evaluated('function f<T: type = uint8>(v: uint8) { return (1 := T) is uint8; } String(f((1 := uint8)));')).toBe('true');
  expect(evaluated('function f<T: type = uint8>(): boolean { return (1 := T) is uint8; } String(f());')).toBe('true');
  // an explicit argument still wins over the default
  expect(evaluated('function f<T: type = uint8>() { return (1 := T) is uint16; } String(f.<uint16>());')).toBe('true');
});

test('a trailing default may be omitted from an argument list', () => {
  expect(evaluated('function f<T: type, U: type = uint8>(v: T) { return (1 := U) is uint8; }'
    + ' String(f.<uint16>((1 := uint16)));')).toBe('true');
  expect(evaluated('class C<T: type, U: type = uint8> { m() { return (1 := U) is uint8; } }'
    + ' String(new C.<uint16>().m());')).toBe('true');
  // a parameter without a default may not be omitted
  expectThrown('class C<T: type, U: type> { m() { return 1; } } new C.<uint8>().m();');
});
...
test('a default may name an earlier parameter', () => {
  ...
  expect(evaluated('function f<T: type, U: type = [].<T>>(v: T) { let a: U = []; return "ok"; }'
    + ' String(f.<uint8>((1 := uint8)));')).toBe('ok');
  expect(evaluated('class C<T: type, U: type = [].<T>> { m() { let a: U = []; return "ok"; } }'
...
test('a declaration whose parameters all have defaults needs no arguments', () => {
  ...
  expect(evaluated('class C<T: type = uint8> { m() { return (1 := T) is uint8; } } String(new C().m());')).toBe('true');
  expect(evaluated('class C<T: type = uint8> { f = (1 := T); } String(new C().f is uint8);')).toBe('true');
  ...
  expect(evaluated('type A<T: type = uint8> = [].<T>; let a: A = []; a.push((1 := uint8)); String(a.length);')).toBe('1');
  expect(evaluated('type A<T: type = uint8, U: type = [].<T>> = U; let a: A = []; String(a.length);')).toBe('0');
  // an explicit argument still overrides the default
  expect(evaluated('class C<T: type = uint8> { m() { return (1 := T) is uint16; } } String(new C.<uint16>().m());')).toBe('true');
});
...
test('a parameter without a default still needs its argument', () => {
  ...
  expectThrown('class C<T: type> { m() { return 1; } } new C();', 'is not determined by the arguments and has no default');
  expect(evaluated('class C<T: type> { m() { return 1; } } String(new C.<uint8>().m());')).toBe('1');
  expectThrown('type A<T: type> = [].<T>; let a: A;');
  expectThrown('type A<T: type, U: type = uint8> = [].<T>; let a: A;');
});
...
test('a specialized field holds the argument type', () => {
  const box = 'class Box<T: type> { value: T; set(v: T) { this.value = v; } } ';
...
test('a plain literal is not a value of the parameter, in a field as in a binding', () => {
  ...
  expectError('class A<T: type> { value: T = 0; }');
  expect(evaluated('class A<T: type> { value: T; constructor(v: T) { this.value = v; } } const a = new A.<uint8>(0);'
...
test('a composed field type substitutes through to its parts', () => {
  ...
  const arr = 'class Arr<T: type> { a: [].<T>; } ';
...
test('two specializations do not share a field type', () => {
  ...
  // they accept.
  const box = 'class Box<T: type> { value: T; } ';
...
test('an unspecialized generic does not construct', () => {
  ...
  expect(evaluated('class U<T: type> { v: T; } const u: U.<uint8> = new U(); typeof u;')).toBe('object');
});

// -- Declaration-site variance (#sec-generic-variance) ------------------------

test('a declared parameter relates its instantiations', () => {
  const decls = 'interface P<out T: type> { get(): T } interface H<in T: type> { put(v: T): void }'
    + ' interface B<T: type> { get(): T } ';
  ...
  const structural = 'type SOut<T: type> = { readonly get: () => T };'
    + ' type SIn<T: type> = { readonly put: (v: T) => void };'
    + ' type SBoth<T: type> = { readonly get: () => T, readonly put: (v: T) => void }; ';
...
test('`out` is contextual and `in` is not', () => {
  ...
  expect(evaluated('type T1<out: type> = out; let a: T1.<uint8> = (1 := uint8); String(a);')).toBe('1');
  ...
  // is not.
  expect(evaluated('interface Q<out out: type> { get(): out } String(typeof (type Q.<uint8>));')).toBe('object');
});
...
test('a variance declaration is checked against its positions', () => {
  ...
  expectThrown('interface Bad<out T: type> { value: T }');       // writable field is ~both~
  expectThrown('interface Bad2<in T: type> { get(): T }');       // return is ~output~
  expectThrown('interface Bad3<out T: type> { put(v: T): void }'); // parameter is ~input~
  // A writable field admits neither modifier, not merely `out`.
  expectThrown('interface Bad4<in T: type> { value: T }');
  // The well-formed ones are untouched.
  expect(evaluated('interface Ok<out T: type> { get(): T; readonly r: T } String(typeof (type Ok.<uint8>));')).toBe('object');
  expect(evaluated('interface Ok2<in T: type> { put(v: T): void } String(typeof (type Ok2.<uint8>));')).toBe('object');
});
