...
test('rung two: a closed SCALAR constraint proposes its inhabitants and exactly one verifies', () => {
  ...
  expect(evaluated('function pick(B) { return B === type true ? uint8 : string; } function one<B: type extends boolean>(x: pick(B)): string { return "bound"; } one(1 := uint8);')).toBe('bound');
  expect(evaluated('function pick(B) { return B === type true ? uint8 : string; } function one<B: type extends boolean>(x: pick(B)): string { return "bound"; } one("s");')).toBe('bound');
});
...
const trialOf = (n: number) => {
  ...
  return `function pick(B) { return B === type ${n - 1} ? uint8 : string; } `
    + `function one<B: type extends ${lits}>(x: pick(B)): string { return "bound"; } one(1 := uint8);`;
};
...
test('rung two: a trial-bound literal is the same Type Object as its written spelling', () => {
  ...
  expect(evaluated('function pick(B) { return B === type true ? uint8 : string; } function one<B: type extends boolean>(x: pick(B)): string { return String(B === type true); } one(1 := uint8);')).toBe('true');
});

test('rung two: a closed PACK constraint proposes its tuples and exactly one verifies', () => {
  expect(evaluated(`${MASK} function withFlags<...Bs: [].<type> extends [2].<boolean>>(m: maskOf(Bs)): string { return String(Reflect.getReflection(Bs).elements.map((e) => e.type === type true).join(",")); } withFlags(1 := uint16);`)).toBe('true,false');
  expect(evaluated(`${MASK} function withFlags<...Bs: [].<type> extends [2].<boolean>>(m: maskOf(Bs)): string { return String(Reflect.getReflection(Bs).elements.map((e) => e.type === type true).join(",")); } withFlags("s");`)).toBe('false,false');
});

test('rung two: no inhabitant, or more than one, refuses naming the builder', () => {
  expectThrown(`${MASK} function withFlags<...Bs: [].<type> extends [2].<boolean>>(m: maskOf(Bs)): uint32 { return 1; } withFlags(true);`, 'no inhabitant');
  expectThrown('function same(B) { return uint8; } function amb<B: type extends boolean>(x: same(B)): uint32 { return 1; } amb(1 := uint8);', 'inhabitants of');
});
