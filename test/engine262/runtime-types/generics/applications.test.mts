...
test('a deferred application annotates a binding and specializes per call', () => {
  ...
  expect(evaluated(`${pairOf} function make<T: type>(x: T) { let p: pairOf(T); return p; }`
...
test('a deferred application enforces its annotation', () => {
  expect(evaluated(`${pairOf} function init<T: type>(x: T) { let p: pairOf(T) = [x, x]; return p; }`
    + ' String(init((7 := uint8)));')).toBe('7,7');
  expectThrown(`${pairOf} function bad<T: type>(x: T) { let p: pairOf(T) = ["wrong", "wrong"]; return p; }`
    + ' bad((1 := uint8));');
  ...
  expectThrown(`${pairOf} function f<T: type>(x: T) { let p: pairOf(T) = [1, 2]; p[0] = "bad"; return p; }`
    + ' f((1 := uint8));');
  expectThrown(`${pairOf} function f<T: type>(x: T) { let p: pairOf(T) = [x, x]; p[0] = "bad"; return p; }`
    + ' f((1 := uint8));');
});

test('a deferred application works in every position a parameter is bound', () => {
  expect(evaluated(`${pairOf} function nested<T: type>(x: T) { let p: pairOf(pairOf(T)); return p; }`
    + ' const r = nested((1 := uint8)); `${r.length}:${r[0].length}`;')).toBe('2:2');
  expect(evaluated(`${pairOf} function param<T: type>(x: T, p: pairOf(T)) { return p.length; }`
    + ' String(param((1 := uint8), [1, 2]));')).toBe('2');
  expect(evaluated(`${pairOf} function ret<T: type>(x: T): pairOf(T) { return [x, x]; }`
...
test('the deferred application record kind exists and relates by identity', () => {
  ...
  expect(evaluated('function widen(T: type): type { return T; } '
    + 'function f<T: type>(v: T): widen(T) { return v; } String(f.<uint8>((1 := uint8)));')).toBe('1');
});
...
test('a checked contract is ASSUMED before specialization', () => {
  ...
  expect(ok(`${omit} function good<T: type>(value: T): omit(T, "password") { return value; }`)).toBe(true);
  ...
  expectThrown(`${omit} function bad<T: type>(value: T): omit(T, "password") { return "no"; }`);
  ...
  const pairOf = 'function pairOf(T) { return Reflect.makeType({ kind: "tuple", '
    + 'elements: [{ type: T, rest: false }, { type: T, rest: false }] }); } ';
  expect(ok(`${pairOf} function f<T: type>(a: pairOf(T)): pairOf(T) { return a; }`)).toBe(true);
});
...
test('a written type argument is checked against its constraint', () => {
  expectStaticTypeError('function f<T: type extends object>(x: T) {} f.<uint8>(1);');
  expectStaticTypeError('class Base {} function h<T: type extends Base>(x: T) {} h.<uint8>(1);');
  // A satisfying argument, and an unconstrained parameter, are unaffected.
  expect(ok('function f<T: type extends object>(x: T) { return x; } f.<object>({});')).toBe(true);
  expect(evaluated('function f<T: type>(x: T) { return x; } String(f.<uint8>(1));')).toBe('1');
  expect(ok('class Base {} class D extends Base {} function h<T: type extends Base>(x: T) { return x; } h.<D>(new D());')).toBe(true);
});
...
test('the applications this does NOT judge', () => {
  ...
  expect(ok('function f<T: type extends object>(x: T) { return x; } f({});')).toBe(true);
});
