 ...
 * the representation, and the equality split that is the whole reason it has
 * to be a PAIR.
 *
 ...
 * three are the same double, so the value is a significand-exponent pair
 * rather than a wrapped double.
 */
...
test('a literal at a decimal type is read from its SOURCE TEXT', () => {
  ...
  //
  // The mechanism is the one bigint literals already use: the checker
...
test('a decimal belongs to the type of its own WIDTH', () => {
...
test('every other literal is UNAFFECTED', () => {
...
test('a decimal field has the IEEE 754 width and alignment', () => {
...
test('a decimal field stores and reads, cohort intact', () => {
