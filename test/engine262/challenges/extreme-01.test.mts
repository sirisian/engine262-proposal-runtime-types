...
test('extreme 216 · Slice', () => {
  const f = `${TUP}\n function slice(T, start, end) { return tupleOf(elementTypes(T).slice(start, end)); }`;
...
test('extreme 476 · Sum', () => {
  const f = `${L}\n function sum(a, b) { return literal((BigInt(a) + BigInt(b)).toString()); }`;
...
test('extreme 517 · Multiply', () => {
  const f = `${L}\n function multiply(a, b) { return literal((BigInt(a) * BigInt(b)).toString()); }`;
...
test('extreme 7561 · Subtract', () => {
  const f = `${L}\n function subtract(a, b) { return a - b < 0 ? never : literal(a - b); }`;
