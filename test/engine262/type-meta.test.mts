...
test('method hooks are name-checked at parse time', () => {
  ...
  expect(run('meta uint8 { subtype(a, b) { return true; } default = 0; frobnicate(v) { return v; } }')).toMatchObject({ Type: 'throw' });
});
...
test('meta: a validate hook decides membership of a parameterized type', () => {
  expect(evaluated(`
    meta float32 { subtype(a, b) { return true; } default = 0; validate(v, m) { return true; } }
    String((1 := float32) is float32.<{ a: 1 }>);
  `)).toBe('true');
  expect(evaluated(`
    meta float32 { subtype(a, b) { return true; } default = 0; validate(v, m) { return false; } }
    String((1 := float32) is float32.<{ a: 1 }>);
  `)).toBe('false');
});
...
test('meta: the hook receives the value and the metadata as an object', () => {
  ...
  expect(evaluated(`
    let seen = "";
    meta float32 { subtype(a, b) { return true; } default = 0; validate(v, m) { seen = typeof m; return true; } }
    let q = (1 := float32) is float32.<{ a: 1 }>;
    seen;
  `)).toBe('object');
  // and its fields are readable, so the verdict can depend on them
  expect(evaluated(`
    meta float32 { subtype(a, b) { return true; } default = 0; validate(v, m) { return m.a === 1; } }
...
test('meta: bounded numerics, the case six capabilities were waiting on', () => {
  ...
  expect(evaluated(`
    meta float32 { subtype(a, b) { return true; } default = 0; validate(v, m) { return Number(v) >= m.min && Number(v) <= m.max; } }
...
test('meta: a meta declaration on a META TYPE governs a parameterization', () => {
  expect(evaluated(`
    ...
    meta Dimensions { subtype(a, b) { return true; } default = 0; validate(v, m) { return false; } }
...
test('meta: a meta type governs a parameterization through the keys it claims', () => {
  expect(evaluated(`
    ...
    meta Bounds { subtype(a, b) { return true; } default = 0; validate(v, m) { return Number(v) >= m.min && Number(v) <= m.max; } }
    String(((5 := float32) is float32.<{ min: 0, max: 10 }>) + "/" + ((50 := float32) is float32.<{ min: 0, max: 10 }>));
  `)).toBe('true/false');
});
...
test('meta: one meta type governs every base that uses its keys', () => {
  ...
  expect(evaluated(`
    type Bounds = { min: float64, max: float64 };
    meta Bounds { subtype(a, b) { return true; } default = 0; validate(v, m) { return Number(v) <= m.max; } }
...
test('meta: two meta types both govern when the metadata uses both their keys', () => {
  expect(evaluated(`
    ...
    meta Lower { subtype(a, b) { return true; } default = 0; validate(v, m) { return Number(v) >= m.lo; } }
    meta Upper { subtype(a, b) { return true; } default = 0; validate(v, m) { return Number(v) <= m.hi; } }
...
test('meta: two meta types may not claim one key', () => {
  ...
  expectThrown(`
    ...
    meta A { subtype(a, b) { return true; } default = 0; }
    meta B { subtype(a, b) { return true; } default = 0; }
  `);
  // distinct keys are fine
  expect(evaluated(`
    type P = { pp: float64 };
    type Q2 = { qq: float64 };
    meta P { subtype(a, b) { return true; } default = 0; }
    meta Q2 { subtype(a, b) { return true; } default = 0; }
    "ok";
  `)).toBe('ok');
});
...
test('meta: a meta type that defines validate still decides normally', () => {
  expect(evaluated(`
    ...
    meta Bound { subtype(a, b) { return true; } default = 0; validate(v, m) { return Number(v) >= m.atLeast; } }
...
test('meta: a hook receives nested metadata as ordinary values', () => {
  ...
  expect(evaluated(`
    type N = { rr: float64 };
    meta N { subtype(a, b) { return true; } default = 0; validate(v, m) { return typeof m.rr === "object" && m.rr.lo === 0; } }
    String((1 := float32) is float32.<{ rr: { lo: 0 } }>);
  `)).toBe('true');
  expect(evaluated(`
    type L = { ll: float64 };
    meta L { subtype(a, b) { return true; } default = 0; validate(v, m) { return Array.isArray(m.ll) && m.ll.length === 2 && m.ll[1] === 2; } }
...
test('meta: a hook is handed a RegExp built from the carried pattern', () => {
  expect(evaluated(`
    type SP = { pat: float64 };
    meta SP { subtype(a, b) { return true; } default = 0; validate(v, m) { return m.pat instanceof RegExp && m.pat.source === "^a" && m.pat.flags === "i"; } }
...
test('meta: the whole-string match a StringPattern meta type would perform', () => {
  ...
  expect(evaluated(`
    ...
    meta SP2 { subtype(a, b) { return true; } default = 0; validate(v, m) { let re = new RegExp("^(?:" + m.rex.source + ")$", m.rex.flags); return re.test(String(v)); } }
...
test('StringPattern: it claims `pattern` globally, as any meta type does', () => {
  ...
  expectThrown(`
    ...
    meta MyPattern { subtype(a, b) { return true; } default = 0; }
  `);
});
...
test('meta: a declaration without a subtype hook is refused', () => {
  expectThrown('type NoSub = { nosub: float64 }; meta NoSub { default = 0; }');
  // and one with it is accepted
  expect(evaluated(`
    type WithSub = { withsub: float64 };
    meta WithSub { subtype(a, b) { return true; } default = 0; }
    "ok";
  `)).toBe('ok');
});
...
test('meta: `validate` stays optional, which is what a brand needs', () => {
  ...
  expect(evaluated(`
    ...
    meta Brandish { subtype(a, b) { return true; } default = 0; }
