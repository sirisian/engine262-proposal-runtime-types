...
// Regression floor for R1: value-type identity. Every assertion here would FAIL
// against pre-R1 code, where TypedNumberValue's inherited NumberValue equality
// made typed numbers compare equal to plain Numbers and to each other across
// types. See correctness-review.md Finding 1 and the remediation plan.
...
test('a typed number is never strictly equal to a plain Number', () => {
  // A LITERAL takes the typed operand's type since F74, so this is "eq". R1 is
  // unchanged and is asserted with a VARIABLE, which adopts nothing: that is
